...
def resolvedRulesZero(rules):
    ...
    while not ruleResolved(rules[0]) > 0 and count < 1000000:
        ruleToAdd = set()
        ruleToRemove = set()
...
if __name__ == '__main__':
    ...
    lineMatchingStar1 = []
    lineToAnalyse = []
    f = open("Z:\donnees\developpement\Python\AdventOfCode\day19_test2.txt", "r")
    for line in f:
        line = line.rstrip("\n")
        if ':' in line:
    ...
    for line in lineToAnalyse:
        ...
        if match:
            if len(line) == len(match.group(0)):
                lineMatchingStar1.append(line)
    print(f"star2 = {len(lineMatchingStar1)}")
